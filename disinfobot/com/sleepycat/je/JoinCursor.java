...
*
* $Id: JoinCursor.java,v 1.1 2004/11/22 18:27:58 kate Exp $
*/
...
public class JoinCursor {
    ...
    JoinCursor(Locker locker,
        ...
        secData = new DatabaseEntry[cursors.length];
     ...
     * violate that rule.
     * <p>
     * A future optimization would be to add a getSearchDup operation and use
     * it instead of repeatedly calling getNextDup.  This would be the
    ...
    private OperationStatus retrieveNext(DatabaseEntry keyParam,
        ...
        outerLoop: while (true) {
            ...
            DatabaseEntry candidateKey = secData[0];
            OperationStatus status;
            if (candidateKey == null) {
                /* Get first duplicate at initial cursor position. */
                candidateKey = new DatabaseEntry();
                secData[0] = candidateKey;
                status = secCursor.getCurrentInternal(secKey, candidateKey,
                                                      lockMode);
            } else {
                /* Already initialized, move to the next candidate key. */
                status = secCursor.retrieveNext(secKey, candidateKey,
                                                lockMode,
                                                GetMode.NEXT_DUP);
            }
            ...
            for (int i = 1; i < secCursors.length; i += 1) {
                ...
                Comparator userCmp = secCursor.getDatabaseImpl()
                                              .getDuplicateComparator();
                DatabaseEntry testKey = secData[i];
                if (testKey == null) {
                    /* Get first duplicate at initial cursor position. */
                    testKey = new DatabaseEntry();
                    secData[i] = testKey;
                    status = secCursor.getCurrentInternal(secKey, testKey,
                                                          lockMode);
                } else {
                    /* Start with the last duplicate read. */
                    status = OperationStatus.SUCCESS;
                }
                while (status == OperationStatus.SUCCESS) {
                    /* Compare key at this cursor to candidate key. */
                    int cmp = (userCmp != null)
                            ? userCmp.compare(testKey.getData(),
                                              candidateKey.getData())
                            : Key.compareByteArray(testKey.getData(),
                                                   candidateKey.getData());
                    if (cmp == 0) {
                        /* Found the candidate value. */
                        break;
                    } else if (cmp > 0) {
                        /* Moved past the candidate value; not found. */
                        status = OperationStatus.NOTFOUND;
                    } else if (cmp < 0) {
                        /* Prior to the candidate value; move forward. */
                        status = secCursor.retrieveNext(secKey, testKey,
                                                        lockMode,
                                                        GetMode.NEXT_DUP);
                    }
                }
