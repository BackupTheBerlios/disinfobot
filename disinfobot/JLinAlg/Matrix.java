...
public class Matrix implements Serializable {
	...
	/**
	 * Constructs a Matrix with a certain number of rows and columns and
	 * and fills it with FieldElements from one FieldElement array.
	 *
	 * @param numberOfRows
	 * @param theEntries in a FieldElement array.
	 * @throws InvalidOperationException if theEntries is null
	 * @throws InvalidOperationException if numberOfRows is not valid
	 */
	...
	public Matrix(FieldElement[] theEntries, int numberOfRows)
		...
		if (theEntries.length % numberOfRows != 0) {
			throw new InvalidOperationException(
				"Tried to construct matrix with "
					+ theEntries.length
					+ " entries and "
					+ numberOfRows
					+ " rows");
	 ...
	 * @throws InvalidOperationException if rowIndex is not between 1 and numberOfRows
	 * or colIndex is not between 1 and numberOfCols
	 */
	...
	public void setRow(int rowIndex, Vector vector)
		...
		if (this.numOfCols != vector.length()) {
			throw new InvalidOperationException(
				"Tried to set row number "
					+ rowIndex
					+ "of a matrix with "
					+ this.numOfCols
					+ " columns with a vector having length "
					+ vector.length()
					+ " ");
		}
	...
	public void setCol(int colIndex, Vector vector) {
		...
		if (this.numOfRows != vector.length()) {
			throw new InvalidOperationException(
				"Tried to set column number "
					+ colIndex
					+ "of a matrix with "
					+ this.numOfRows
					+ " rows with a vector having length "
					+ vector.length()
					+ " ");
		}
	...
	public Matrix insertCol(int colIndex, Vector vector)
		...
		if (this.getRows() != vector.length()) {
			throw new InvalidOperationException(
				"This vector\n"
					...
					+ "the Matrix\n"
					+ this
					+ " as a column vector. The length does "
					+ "not match");
		}
		...
		if (colIndex < 1 || colIndex > this.getCols() + 1) {
			throw new InvalidOperationException(
				colIndex
					+ " is not a valid column index for inserting "
					+ vector
					+ " into\n"
					+ this);
		}
	...
	public Matrix insertRow(int rowIndex, Vector vector)
		...
		if (rowIndex < 1 || rowIndex > this.getRows() + 1) {
			throw new InvalidOperationException(
				rowIndex
					+ " is not a valid row index for inserting "
					+ vector
					+ " into\n"
					+ this);
		}
	...
	public Matrix add(Matrix anotherMatrix) throws InvalidOperationException {
		...
		return operate(this, anotherMatrix, new AddOperator(), "sum");
	}
	...
	public Matrix subtract(Matrix anotherMatrix)
		throws InvalidOperationException {
		return operate(this, anotherMatrix, new SubtractOperator(), "diff");
	}
	...
	public FieldElement det() throws InvalidOperationException {
		if (this.getRows() != this.getCols()) {
			throw new InvalidOperationException("Square matrix needed for determinant");
		}
		return detCalc();
	}
			...
			for (int j = 1; j <= tmp.getRows(); j++) {
				...
				for (int k = colCounter; k <= tmp.getCols(); k++) {
					...
					tmp.set(
						j,
						k,
						oldEntry.subtract(tmp.get(row, k).multiply(factor)));
				}
			}
		}
		return tmp;
	...
	public Matrix gausselim() {
		...
		while (row < minOfRowsCols && colCounter < tmp.getCols()) {
			...
			for (int j = row; j <= tmp.getRows(); j++) {
				...
				FieldElement factor =
					tmp.get(j, colCounter).divide(diagonalEntry);
				if (row == j || factor.isZero()) {
					continue;
				}
				...
				for (int k = colCounter; k <= tmp.getCols(); k++) {
					...
					tmp.set(
						j,
						k,
						oldEntry.subtract(factor.multiply(tmp.get(row, k))));
				}
			}
		}
		return tmp;
	 ...
	 * @throws InvalidOperationException if this matrix is no square matrix
	 * or the entries are not all DoubleWrappers.
	 */
			...
			for (int j = 1; j <= tmp.getRows(); j++) {
				...
				for (int k = 1; k <= tmp.getCols(); k++) {
					...
					tmp2.set(
						j,
						k,
						oldEntry2.subtract(tmp2.get(row, k).multiply(factor)));
				}
			}
		}
		...
		for (int i = 1; i <= matrix3.getRows(); i++) {
			for (int j = 1; j <= matrix3.getCols(); j++) {
				matrix3.set(
					i,
					j,
					fun.apply(matrix1.get(i, j), matrix2.get(i, j)));
			}
		}
		return matrix3;
	}
