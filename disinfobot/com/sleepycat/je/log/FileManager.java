...
*
* $Id: FileManager.java,v 1.1 2004/11/22 18:27:58 kate Exp $
*/
 ...
 * out LSNs.
 *
 * @version $Id: FileManager.java,v 1.1 2004/11/22 18:27:58 kate Exp $
 */
public class FileManager {
    ...
    private DbLsn nextAvailableLsn;  // nextLSN is the next one available
    private DbLsn lastUsedLsn;       // last LSN used in the current log file
     ...
     */
    private DbLsn savedNextAvailableLsn; // nextLSN is the next one available
    private DbLsn savedLastUsedLsn;  // last LSN used in the current log file
    ...
    public FileManager(EnvironmentImpl envImpl,
        ...
        currentFileNum = 0L;
        nextAvailableLsn = new DbLsn(currentFileNum, 
                                     firstLogEntryOffset());
        lastUsedLsn = null;
     ...
     */
    public void setLastPosition(DbLsn nextAvailableLsn,
                                DbLsn lastUsedLsn,
                                long prevOffset) {
        this.lastUsedLsn = lastUsedLsn;
        perFileLastUsedLsn.put(new Long(lastUsedLsn.getFileNumber()),
                               lastUsedLsn);
        this.nextAvailableLsn = nextAvailableLsn;
        currentFileNum = this.nextAvailableLsn.getFileNumber();
        this.prevOffset = prevOffset;
	saveLastPosition();
    }

    /*
     * Cause the current LSN state to be saved in case a log IO throws an
     * IOException.  If an IOE is thrown, then we can restore the old values.
     */
    ...
    private String getFileName(long fileNum, String suffix) {
        /* 
         * HexFormatter generates a 0 padded string starting with 0x.
         * We want the right most 8 digits, so start at 10.
         */
            ...
            if (fileHandle == null) {
                ...
                try {
                    ...
                    if (fileHandle == null) {
                        ...
                        fileHandle = makeFileHandle(fileNum, READ_MODE);
    ...
    private FileHandle makeFileHandle(long fileNum, String mode)
        throws DatabaseException {
	     ...
	     */
            ...
            if (newFile.length() == 0) {
                ...
                DbLsn lastLsn = (DbLsn)
                    perFileLastUsedLsn.remove(new Long(fileNum - 1));
                long headerPrevOffset = 0;
                if (lastLsn != null) {
                    headerPrevOffset = lastLsn.getFileOffset();
                }
            ...
            return new FileHandle(newFile, fileName, envImpl);
        } catch (FileNotFoundException e) {
     ...
     */
    private void readAndValidateFileHeader(RandomAccessFile file,
                                           String fileName,
                                           long fileNum)
        throws DatabaseException, IOException {
        ...
        LogEntry headerEntry = 
            logManager.getLogEntry(new DbLsn(fileNum, 0), file);
        FileHeader header = (FileHeader) headerEntry.getMainItem();
        header.validate(fileName, fileNum);
    }
    ...
    private void writeFileHeader(RandomAccessFile file,
        ...
        ByteBuffer headerBuf =
            envImpl.getLogManager().putIntoBuffer(header,
                                                  headerSize,
                                                  0, 
                                                  false,
                                                  entrySize);
	...
	if (++writeCount >= stopOnWriteCount) {
        ...
        if (bytesWritten != entrySize) {
            throw new LogException("File " + fileName +
                                     " was create with an incomplete header." +
                                     " Only " + bytesWritten +
                                     " bytes written");
        }
    }
    ...
    long getFileHeaderPrevOffset(long fileNum) 
        ...
        LogEntry headerEntry = 
            envImpl.getLogManager().getLogEntry(new DbLsn(fileNum, 0));
        FileHeader header = (FileHeader) headerEntry.getMainItem();
    ...
    boolean bumpLsn(long size) {
        ...
        if ((nextAvailableLsn.getFileOffset() + size) > maxFileSize) {
            ...
            prevOffset = 0;
            lastUsedLsn = new DbLsn(currentFileNum, 
                                    firstLogEntryOffset());
            flippedFiles = true;
        } else {
            if (lastUsedLsn == null) {
                ...
                prevOffset = lastUsedLsn.getFileOffset();
            }
            lastUsedLsn = nextAvailableLsn;
        }
        nextAvailableLsn = new DbLsn(lastUsedLsn.getFileNumber(),
                                     (lastUsedLsn.getFileOffset() + size));
    ...
    void writeLogBuffer(LogBuffer fullBuffer)
        ...
        DbLsn firstLsn = fullBuffer.getFirstLsn();
        ...
        if (firstLsn != null) {
            ...
            FileChannel channel =
                endOfLog.getWriteChannel(firstLsn.getFileNumber());
            ...
            try {
                 ...
                 * of an interrupt received by another thread. See SR [#10463].
                 */
                channel.write(data, firstLsn.getFileOffset());
            } catch (ClosedChannelException e) {
                throw new RunRecoveryException(envImpl,
                                               "Channel closed, may be "+
                                               "due to thread interrupt",
		...
		/*
		 * Possibly an out of disk exception.  Truncate the log back to
		 * the start of this buffer that just failed, restore the state
		 * of the current position, rethrow the exception and let the
		 * user decide whether they want to keep going or not.
		 */
		truncateLog(firstLsn.getFileNumber(),
			    firstLsn.getFileOffset());
		restoreLastPosition();
		throw IOE;
	    }
	...
	if (envDirIsReadOnly &&
	    !readOnly) {
	    throw new DatabaseException
		("The Environment directory is not writable, but the " +
    ...
    public void truncateLog(long fileNum, long offset) 
        ...
        RandomAccessFile file =
            makeFileHandle(fileNum, READWRITE_MODE).getFile();
        try {
    ...
    class LogEndFileDescriptor {
        ...
        /** 
         * getWriteChannel must be called under the log write latch. 
         */
        FileChannel getWriteChannel(long fileNumber) 
            throws DatabaseException {
            ...
            if (endOfLogRWFile == null) {
                 ...
                 * This is guaranteed to be called under the log write latch.
                 */
                endOfLogRWFile =
                    makeFileHandle(fileNumber, READWRITE_MODE).getFile();
                endOfLogSyncFile =
                    makeFileHandle(fileNumber, READWRITE_MODE).getFile();
            }
            
            return endOfLogRWFile.getChannel();
        }
        ...
        void close()
            ...
            if (endOfLogRWFile != null) {
                ...
                /* 
                 * Null out so that other threads know endOfLogRWFile is
                 * no longer available.
                 */
                endOfLogRWFile = null;
                try {
