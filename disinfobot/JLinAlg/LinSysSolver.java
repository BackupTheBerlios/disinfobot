...
public class LinSysSolver implements Serializable{
	...
	/**
	 * Calculates the solution space of a given linear equation system of the form A*x=b.
	 *
	...
	public static AffineLinearSubspace solutionSpace(Matrix a, Vector b)
		...
		Vector[] generatingSystem =
			new Vector[extCoeff.getCols() - extCoeff.getRows() - 1];

		for (int col = extCoeff.getRows() + 1;
			col <= extCoeff.getCols() - 1;
			col++) {
		...
		for (int row = 1; row <= dimension; row++) {
			if (row <= extCoeff.getRows()) {
				inhomogenousPart.set(
					row,
					extCoeff.get(row, extCoeff.getCols()));
			} else {
	...
	/**
	 * Calculates a solution of a given linear equation system of the form A*x=b.
	 *
	 * @param a coefficient matrix
	 * @param b result vector
	...
	/**
	 * Does something quite similar to isSolvable but is used by solve and solutionSpace
	 * 
	 * @param a coefficient matrix
	 * @param b result vector
	...
	private static Matrix isSolvableHelper(Matrix a, Vector b) {
		...
		//Check whether there is any non zero entry in the first not zero row of tmp 
	 ...
	 * @param a coefficient matrix
	 * @param b result vector
	 * @return return true if and only if there is a solution for this linear eqution system.
	 */
