...
*
* $Id: SecondaryCursor.java,v 1.1 2004/11/22 18:27:59 kate Exp $
*/
...
public class SecondaryCursor extends Cursor {
     ...
     */
    SecondaryCursor(SecondaryDatabase dbHandle, Transaction txn,
                    CursorConfig cursorConfig, SecondaryConfig secConfig) 
    ...
    private OperationStatus getCurrentInternal(DatabaseEntry key,
        ...
        if (status == OperationStatus.SUCCESS) {
            /* May return KEYEMPTY if dirty-read and the primary was deleted.*/
    ...
    OperationStatus search(DatabaseEntry key,
        ...
        /* Perform retries to account for deletions during a dirty-read. */
        while (true) {
     ...
     * getCurrent).  KEYEMPTY is only returned when dirty-read is used.
     *
     * @return SUCCESS if the primary was read succesfully, or KEYEMPTY if
     * using dirty-read and the primary has been deleted, or KEYEMPTY if using
     * dirty read and the primary has been updated and no longer contains the
     * secondary key.
     *
     * @throws DatabaseException to indicate a corrupt secondary reference if
     * the primary record is not found and dirty-read is not used (or
     * dirty-read is used, but we cannot verify that a valid deletion has
     * occured).
     */
    private OperationStatus readPrimaryAfterGet(DatabaseEntry key,
	    ...
	    cursor = new Cursor(primaryDb, locker, null);
            ...
            if (status != OperationStatus.SUCCESS) {
                /*
                 * If using dirty-read and the primary is not found, check to
                 * see if the secondary key also has been deleted.  If so, the
                 * primary was deleted in between reading the secondary and the
                 * primary.  It is not corrupt, so we return KEYEMPTY.
                 */
                if (cursorImpl.isDirtyReadMode(lockMode)) {
            ...
            /*
             * If using dirty-read and the primary was found, check to see if
             * primary was updated so that it no longer contains the secondary
             * key.  If it has been, return KEYEMPTY.
             */
            if (cursorImpl.isDirtyReadMode(lockMode)) {
                DatabaseEntry secKey = new DatabaseEntry();
                if (!keyCreator.createSecondaryKey(secondaryDb, pKey, data,
                                                   secKey) ||
                    !secKey.dataEquals(key)) {
